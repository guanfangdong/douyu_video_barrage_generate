...
class Separator(object):
    ...
    def separate_to_file(
        ...
        duration_times = int(duration // 600)

        for i in range(duration_times):
            
            offset = 600 * i
            filename_format: str = "{filename}/{instrument}_%d.{codec}" %i
            duration = 600.0

            print(i, duration_times, offset, filename_format)
        ...
        if duration > duration_times * 600:
            
            offset = 600 * duration_times
            filename_format: str = "{filename}/{instrument}_%d.{codec}" %duration_times
            duration = duration - duration_times * 600
