...
def load_barrage(filename):
    ...
    for line in f.readlines():
        ...
        if "&&&" in line:
            each_barrage = line.split("&&&")
            barrage_list.append(each_barrage)
    ...
    def take_first(elem):
        ...
        second = int(parts[1])
        return minute*60+second
    barrage_list.reverse()
    barrage_list.sort(key=take_first)
    f.close()
    return barrage_list
...
if __name__ == "__main__":
    while True:
        title = input("输入标题：")
        barrage_list = load_barrage(title)
        length = to_second(barrage_list[-1][0])
        gap = 10
        how_many = length // gap + 1
        count_list = [0] * how_many
        for i in barrage_list:
            second = to_second(i[0])
            which_interval = second // gap
            count_list[which_interval] += 1
        ave = sum(count_list) / len(count_list)
        gaoneng_thres = ave * 3
        gaoneng_list = []
        print("aveage is %d" %int(ave))
        for step, i in enumerate(count_list):
            if i > gaoneng_thres:
                start = to_min(step * gap)
                end = to_min((step + 1) * gap)
                gaoneng_list.append("from %s to %s has %d danmu" %(start, end, i))
        for i in gaoneng_list:
            print(i)
        print("完成现在开始下一个")
